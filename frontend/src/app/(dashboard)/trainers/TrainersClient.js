...
export default function TrainersClient({ initialTrainers, initialPlans, initialPaidSalaries }) {
  ...
  const [paidSalaries, setPaidSalaries] = useState(initialPaidSalaries || []);
  ...
  const handlePaySalaryClick = (t) => {
    let fixedSalary = t.salary || 0;
    if (user?.trainerCompensation) {
      const compSettings = user.trainerCompensation;
        ...
        {loading ? null : trainers.length === 0 ? (
          ...
          {filteredTrainers.length === 0 ? (
            ...
            <div className="block md:hidden space-y-3 max-h-[360px] overflow-y-auto pb-4 pr-1">
              {filteredTrainers.map((t, idx) => {
                const isExpanded = expandedTrainerId === t._id;
                const initials = t.name ? t.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : 'T';
                const isPaid = isTrainerPaidThisMonth(t.name);
                ...
                return (
                  <div
                    ...
                    className={`border border-white/5 rounded-2xl transition-all ${
                      isInactive ? 'opacity-50 grayscale-[0.8]' : ''
                    } ${
                  ...
                  >
                    ...
                    <div
                      onClick={() => setExpandedTrainerId(isExpanded ? null : t._id)}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] font-bold text-text-muted uppercase tracking-wider mt-0.5">
                            {calculateExperience(t.experienceStartDate)}Y Exp • {t.trainerType || 'Trainer'}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="px-4 pb-4 pt-1 border-t border-white/5 space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-[10px]">
                          <div>
                            ...
                            <a href={`tel:${t.phone}`} className="text-white font-extrabold hover:underline">
                              {t.phone || 'N/A'}
                            </a>
                          </div>
                          <div>
                            <span className="text-text-muted font-bold block uppercase tracking-wider text-[8px] mb-0.5">Salary</span>
                            <span className="text-white font-extrabold">
                              ₹{(t.salary || 0).toLocaleString()} {t.commission > 0 && `(+${t.commission}% PT)`}
                            </span>
                          </div>
                          <div>
                            <span className="text-text-muted font-bold block uppercase tracking-wider text-[8px] mb-0.5">Shift Hours</span>
                            <span className="text-white font-extrabold uppercase">
                              {formatTime12(t.shiftStart || '06:00')} - {formatTime12(t.shiftEnd || '22:00')}
                            </span>
                          </div>
                          <div>
                            <span className="text-text-muted font-bold block uppercase tracking-wider text-[8px] mb-0.5">Specialties</span>
                            <span className="text-white font-extrabold truncate block max-w-[140px]" title={t.specialties?.join(', ')}>
                              {t.specialties?.join(', ') || 'N/A'}
                            </span>
                          </div>
                          {t.trainerType !== 'Normal Trainer' && (
                            <div className="col-span-2">
                              <span className="text-text-muted font-bold block uppercase tracking-wider text-[8px] mb-0.5">Active PT Clients</span>
                              <div
                                onClick={() => { if ((t.clientCount || 0) > 0) setViewingClients(t); }}
                                className={`flex items-center gap-1.5 w-fit ${
                                  (t.clientCount || 0) > 0 ? 'text-accent cursor-pointer hover:underline' : 'text-text-muted'
                                }`}
                              >
                                <Users size={12} className="text-accent" />
                                <span className="font-extrabold">{t.clientCount || 0} clients</span>
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          ...
                          <button
                            disabled={isPaid}
                            onClick={() => handlePaySalaryClick(t)}
                            className={`flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all cursor-pointer ${
                              isPaid
                                ? 'bg-success/5 border-success/15 text-success/60 cursor-not-allowed'
                                : 'bg-success/15 text-success hover:bg-success hover:text-white border-success/20'
                            }`}
                          >
                            <Banknote size={12} />
                          ...
                          <button
                            ...
                            className={`w-9 h-7 flex items-center justify-center rounded-lg border transition-all cursor-pointer ${
                              t.status !== 'inactive'
                            ...
                            title={t.status !== 'inactive' ? 'Deactivate' : 'Activate'}
                          >
                            {t.status !== 'inactive' ? <UserMinus size={12} /> : <UserCheck size={12} />}
                          </button>
                          <button
      ...
      {openMenuId && activeMenu && (
        ...
        >
          <div className="p-2 space-y-1">
            ...
            <button
              onClick={() => {
                handlePaySalaryClick(activeMenu.data);
                setOpenMenuId(null);
                setActiveMenu(null);
              }}
