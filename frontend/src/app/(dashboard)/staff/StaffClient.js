...
export default function StaffClient({ initialStaff, initialExpenses }) {
  ...
  const [salaryExpenses, setSalaryExpenses] = useState(initialExpenses || []);
          ...
          {filteredStaff.length === 0 ? (
            ...
            <div className="block md:hidden space-y-3 max-h-[360px] overflow-y-auto pb-4 pr-1">
              {filteredStaff.map((s, idx) => {
                ...
                const initials = s.name ? s.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : 'S';
                const isPaid = isStaffPaidThisMonth(s);
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      onClick={() => setExpandedStaffId(isExpanded ? null : s._id)}
                      ...
                      <div className="px-4 pb-4 pt-1 border-t border-white/5 space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-[10px]">
                          <div>
                            ...
                            <a href={`tel:${s.phone}`} className="text-white font-extrabold hover:underline">
                              {s.phone || 'N/A'}
                            </a>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <span className="text-white font-extrabold uppercase">
                              {formatTime12(s.shiftStart || '06:00')} - {formatTime12(s.shiftEnd || '22:00')}
                            </span>
                          </div>
                          <div>
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          ...
                          <button
                            ...
                            onClick={() => {
                              setSalaryForm({
                                amount: String(s.salary || ''),
                                category: 'Salary',
                                titleSelect: `${s.name} - ${new Date().toLocaleString('en-US', { month: 'long' })} Salary [Salary]`,
                                customTitle: '',
                                date: new Date().toISOString().split('T')[0],
                                notes: `Staff salary payout for ${s.name}`,
                                paymentMethod: 'cash'
                              });
                              setSalaryStaff(s);
                            }}
                          ...
                          <button
                            ...
                            title={s.status !== 'inactive' ? 'Deactivate' : 'Activate'}
                          >
                            {s.status !== 'inactive' ? <UserMinus size={12} /> : <UserCheck size={12} />}
                          </button>
                          <button
