...
const categoryTitlesMapping = {
  Salary: ["Trainer Salary", "Staff Salary", "Housekeeping Salary", "Other"],
  Bonus: ["Performance Bonus", "Festival Bonus", "Other"],
  Advance: ["Salary Advance", "Other"]
};
...
export default function SalariesTab() {
  ...
  const handleDelete = (id) => {
    setDeleteConfirmState({
      ...
      message: 'Are you sure you want to delete this salary payment record? This action cannot be undone.',
      onConfirm: async () => {
        try {
          await expensesApi.delete(id);
          fetchData(true);
          showToast(' Salary payment deleted successfully!', 'success');
        } catch (err) { showToast(err.message, 'error'); }
      }
    });
  };
  ...
  const handleCategoryChange = (cat) => {
    const defaultTitle = categoryTitlesMapping[cat] ? categoryTitlesMapping[cat][0] : 'Other';
    setForm(prev => ({
      ...prev,
      category: cat,
  ...
  const dynamicStats = (() => {
    ...
    // Calculate Bonus and Advance totals for the selected active month
    const bonusTotal = activeMonthExpenses
      .filter(e => e.title && e.title.includes('[Bonus]'))
      .reduce((acc, curr) => acc + curr.amount, 0);

    const advanceTotal = activeMonthExpenses
      .filter(e => e.title && e.title.includes('[Advance]'))
      .reduce((acc, curr) => acc + curr.amount, 0);
      ...
      <div className="bg-bg-card border border-white/5 rounded-xl shadow-2xl flex flex-col">
        ...
        <div className="py-4 px-6 border-b border-white/5 space-y-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-black text-text-primary tracking-tight">Salaries</h1>
              <div className="flex items-center gap-2 text-[10px] font-black text-text-muted uppercase tracking-widest mt-1 opacity-70">
                ...
                Track staff & trainer payroll
              </div>
            </div>

            {/* Calendar next to Record Expense */}
            <div className="flex items-center gap-3">
              <SearchBar value={search} onChange={setSearch} placeholder="Search salaries..." />
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <StatCard
              icon={<Zap />}
              label="Bonuses Paid"
              value={`₹${dynamicStats.bonusTotal.toLocaleString()}`}
              trend="Bonus pay"
              trendUp={true}
              subtitle={`Paid in ${dynamicStats.monthlyLabel}`}
              size="xs"
            ...
            <StatCard
              icon={<IndianRupee />}
              label="Advances"
              value={`₹${dynamicStats.advanceTotal.toLocaleString()}`}
              trend="Advance pay"
              trendUp={false}
              subtitle={`Paid in ${dynamicStats.monthlyLabel}`}
              size="xs"
        ...
        {filteredExpenses.length === 0 ? (
          <EmptyState
            icon={<Landmark size={48} className="text-text-muted opacity-50" />}
          ...
          <>
            ...
            <thead className="sticky top-0 bg-[#0a0a0a] z-10 border-b border-white/5 shadow-md">
              <tr className="bg-white/[0.02]">
                ...
                <SortHeader label="Salary Info" sortKey="title" />
                <th className="px-6 py-2.5 text-[10px] font-black text-text-muted uppercase tracking-[0.2em]">Category</th>
                <th className="px-6 py-2.5 text-[10px] font-black text-text-muted uppercase tracking-[0.2em]">Pay Month</th>
      ...
      >
        <form onSubmit={handleAdd} className="space-y-6">
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                searchable={false}
                options={(categoryTitlesMapping[form.category] || []).map(title => ({
                  label: title,
                  value: title
                }))}
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
              <div className="space-y-1">
                ...
                <input placeholder="Amount ₹ *"
                  ...
                  required
                  className="[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none w-full !h-8  !text-[15px] !bg-bg-card/40 border border-white/5 !rounded-xl text-white outline-none focus:border-accent/50 px-4 transition-all"
                />
              </div>
              <div className="space-y-1">
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {saving ? 'Processing...' : 'Record Salary'}
            </button>
          </div>
        </form>
      </Modal>
