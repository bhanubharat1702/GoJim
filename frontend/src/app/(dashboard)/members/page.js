...
export default function MembersPage() {
  ...
  const [paymentMap, setPaymentMap] = useState({}); // { memberId: latestPaidPayment }
  ...
  const filteredMembers = useMemo(() => {
    ...
    return members.filter(m => {
      ...
      if (statusFilter !== 'all') {
        ...
        // No payment on record → always expired
        const isExpired = !effectiveExpiry || effectiveExpiry < now;
        const diffDays = effectiveExpiry ? Math.floor(Math.abs(now - effectiveExpiry) / (1000 * 60 * 60 * 24)) : null;
        const isDueSoon = !isExpired && effectiveExpiry && diffDays <= 3;
        const isJoinedToday = joinDate && joinDate.toDateString() === now.toDateString();
        const isExpiringToday = !isInactive && effectiveExpiry && effectiveExpiry.toDateString() === now.toDateString();

        if (statusFilter === 'inactive' && !isInactive) return false;
        if (statusFilter === 'expired' && (!isExpired || isInactive)) return false;
        if (statusFilter === 'due_soon' && (!isDueSoon || isInactive)) return false;
        if (statusFilter === 'expiring_today' && (!isExpiringToday || isInactive)) return false;
        if (statusFilter === 'paid' && (isExpired || isDueSoon || isInactive || isJoinedToday || isExpiringToday)) return false;
        if (statusFilter === 'joined_today' && (!isJoinedToday || isInactive)) return false;
      }
  ...
  }, [members, paymentMap, statusFilter, dateFilterType, selectedDate, selectedMonth, selectedYear, selectedRangeStart, selectedRangeEnd]);
  ...
  const fetchMembers = async (isLoadMore = false, isSilent = false) => {
    try {
      ...
      const [membersRes, statsRes, trainRes, planRes, paidPaymentsRes] = await Promise.all([
        filter === 'inactive' ? membersApi.getInactive(query) : filter === 'expiring' ? membersApi.getExpiring(query) : membersApi.getAll(`${query}&status=${filter}`),
        !isLoadMore ? membersApi.getStats() : Promise.resolve({ success: false }),
        !isLoadMore ? trainersApi.getAll('status=active') : Promise.resolve({ success: false }),
        !isLoadMore ? plansApi.getAll() : Promise.resolve({ success: false }),
        paymentsApi.getAll('status=paid&limit=10000')
      ]);
      ...
      // Build memberId -> latest paid payment map for status verification
      if (paidPaymentsRes?.success && paidPaymentsRes.data) {
        const map = {};
        paidPaymentsRes.data.forEach(p => {
  ...
  useEffect(() => { fetchMembers(); }, [filter, planFilter, genderFilter, debouncedSearch, sortBy]);
